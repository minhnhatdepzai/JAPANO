...
// /api/state và /api/seed giờ yêu cầu admin thật (xem backend/lib/auth.js) —
// đăng nhập bằng JAPANO_ADMIN_EMAIL/PASSWORD trong .env.server nếu có, còn
// không thì bỏ qua hai kiểm tra đó thay vì làm hỏng toàn bộ script.
let adminToken = '';
...
async function main() {
  ...
  const adminEmail = process.env.JAPANO_ADMIN_EMAIL || '';
  const adminPassword = process.env.JAPANO_ADMIN_PASSWORD || '';
  if (adminEmail && adminPassword) {
    try {
      ...
      console.warn(`Không đăng nhập admin được (${error.message}) — bỏ qua kiểm tra /api/state và /api/seed.`);
    }
  }
  ...
  if (adminToken) {
    ...
    state = await request('/api/products').then((products) => ({ products }));
  }
