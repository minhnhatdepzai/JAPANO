export const V45_API_BASE =
  (typeof process !== 'undefined' && process.env?.EXPO_PUBLIC_API_URL)
    ? String(process.env.EXPO_PUBLIC_API_URL)
    : 'http://localhost:4000';

export function v45FirstImage(product: any): string {
  if (!product) return '';
...
export function v45ProductId(product: any): string {
  return String(product?._id || product?.id || product?.sku || product?.slug || product?.name || '');
}

export async function v45Get(path: string) {
  const res = await fetch(`${V45_API_BASE}${path}`);
...
export async function v45Post(path: string, body: any) {
  const res = await fetch(`${V45_API_BASE}${path}`, {
    method: 'POST',
