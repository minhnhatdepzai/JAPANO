...
export const api = {
  ...
  createStripeCheckoutSession: (body: any) => request('/api/stripe/create-checkout-session', { method: 'POST', body: JSON.stringify(body) }),
  getSystemStatus: (adminId: string) => request(`/api/admin/system-status?adminId=${encodeURIComponent(adminId)}`),
