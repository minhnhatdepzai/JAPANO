...
window.A={
  ...
  revPoint(span,index){const row=revBy(span)[index]||{},detail=revenuePeriodOrders(span,index),orders=detail.orders,total=orders.reduce((sum,o)=>sum+orderAmount(o),0);openModal(`<div class="mh"><div><h3>Chi tiết doanh thu · ${esc(row.label||'Kỳ đã chọn')}</h3><div class="faint" style="font-size:11.5px">${detail.start.toLocaleDateString('vi-VN')} – ${new Date(+detail.end-1).toLocaleDateString('vi-VN')} · ${state.dataScope==='live'?'dữ liệu thực':'gồm dữ liệu mẫu'}</div></div><div class="x" onclick="closeModal()">${icon('close',17)}</div></div><div class="mb"><div class="metriccards"><div class="metriccard"><div class="ml">Doanh thu</div><div class="mv">${money(total)}</div></div><div class="metriccard"><div class="ml">Số đơn</div><div class="mv">${orders.length}</div></div><div class="metriccard"><div class="ml">Trung bình/đơn</div><div class="mv">${money(orders.length?total/orders.length:0)}</div></div></div><div class="tablewrap" style="margin-top:14px"><table class="tbl"><thead><tr><th>Mã đơn</th><th>Khách hàng</th><th>Thời gian</th><th class="right">Số tiền</th></tr></thead><tbody>${orders.length?orders.sort((a,b)=>orderTime(b)-orderTime(a)).map(o=>`<tr onclick="closeModal();A.openOrder('${escJs(o.id)}')" style="cursor:pointer"><td class="bold">#${esc(o.code)}</td><td>${esc(o.customer?.name||o.userId||'Khách')}</td><td>${fmtDate(orderTime(o))}</td><td class="right bold">${money(orderAmount(o))}</td></tr>`).join(''):emptyTR(4,'Kỳ này chưa có đơn tạo doanh thu.')}</tbody></table></div></div>`,`lg`);},
  revenueSummary(){const windowData=revenueWindow(state.revSpan),orders=windowData.orders,total=orders.reduce((sum,o)=>sum+orderAmount(o),0),spanName={day:'7 ngày',week:'8 tuần',month:'12 tháng',year:'5 năm'}[state.revSpan]||'khoảng đang xem';openModal(`<div class="mh"><h3>Tổng hợp doanh thu · ${spanName}</h3><div class="x" onclick="closeModal()">${icon('close',17)}</div></div><div class="mb"><div class="metriccards"><div class="metriccard"><div class="ml">Tổng doanh thu</div><div class="mv">${money(total)}</div></div><div class="metriccard"><div class="ml">Đơn có doanh thu</div><div class="mv">${orders.length}</div></div><div class="metriccard"><div class="ml">Giá trị trung bình</div><div class="mv">${money(orders.length?total/orders.length:0)}</div></div></div><p class="faint" style="margin-top:14px;font-size:11.5px">Khoảng biểu đồ: ${windowData.start.toLocaleDateString('vi-VN')} – ${new Date(+windowData.end-1).toLocaleDateString('vi-VN')}. Chỉ tính đơn đã thanh toán hoặc hoàn tất; loại đơn huỷ, trả hàng, thất bại và hoàn tiền. Phạm vi: ${state.dataScope==='live'?'dữ liệu phát sinh thực':'toàn bộ, gồm dữ liệu mẫu'}.</p><div class="tablewrap" style="margin-top:14px"><table class="tbl"><thead><tr><th>Mã đơn</th><th>Khách hàng</th><th>Thời gian</th><th class="right">Số tiền</th></tr></thead><tbody>${orders.length?[...orders].sort((a,b)=>orderTime(b)-orderTime(a)).map(o=>`<tr onclick="closeModal();A.openOrder('${escJs(o.id)}')" style="cursor:pointer"><td class="bold">#${esc(o.code)}</td><td>${esc(o.customer?.name||o.userId||'Khách')}</td><td>${fmtDate(orderTime(o))}</td><td class="right bold">${money(orderAmount(o))}</td></tr>`).join(''):emptyTR(4,'Khoảng này chưa có đơn tạo doanh thu.')}</tbody></table></div></div>`,`lg`);},
  orderDrill(status){let orders=dashboardOrders();if(status==='paid')orders=orders.filter(revenueOrder);else if(status==='shipping')orders=orders.filter(o=>['shipping','confirmed'].includes(o.status));else if(status!=='all')orders=orders.filter(o=>o.status===status);openModal(`<div class="mh"><div><h3>Danh sách đơn · ${status==='all'?'Tất cả':status==='paid'?'Có doanh thu':ORD[status]?.t||status}</h3><div class="faint" style="font-size:11.5px">${orders.length} đơn · nhấn một dòng để mở chi tiết</div></div><div class="x" onclick="closeModal()">${icon('close',17)}</div></div><div class="tablewrap"><table class="tbl"><thead><tr><th>Mã</th><th>Khách</th><th>Ngày</th><th>Trạng thái</th><th class="right">Tổng</th></tr></thead><tbody>${orders.length?orders.sort((a,b)=>orderTime(b)-orderTime(a)).map(o=>`<tr onclick="closeModal();A.openOrder('${escJs(o.id)}')" style="cursor:pointer"><td class="bold">#${esc(o.code)}</td><td>${esc(o.customer?.name||'Khách')}</td><td>${fmtDate(orderTime(o))}</td><td>${badge(ORD,o.status)}</td><td class="right bold">${money(orderAmount(o))}</td></tr>`).join(''):emptyTR(5,'Không có đơn phù hợp.')}</tbody></table></div>`,`lg`);},
  ...
  // Tương thích với HTML Admin cũ còn cache: tên hàm cũ cũng chỉ được ẩn,
  // tuyệt đối không gửi DELETE và không loại sản phẩm khỏi dữ liệu cục bộ.
  delProduct(id){const p=DB.products.find(x=>x.id===id);if(p&&p.status!=='hidden')return A.toggleHide(id);},
  async toggleHide(id){const p=DB.products.find(x=>x.id===id);if(!p)return;const status=p.status==='hidden'?'published':'hidden',payload={...p,status};try{const result=await requestJSON('/products/'+encodeURIComponent(id),30000,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});Object.assign(p,result.product);localStorage.setItem(LS,JSON.stringify(DB));renderView();toast(p.status==='hidden'?'Đã ẩn sản phẩm · dữ liệu vẫn được giữ':'Đã hiển thị lại sản phẩm');}catch(e){toast('Không cập nhật được sản phẩm','err');}},
  ...
  aiDesc(){const t=$('#f-desc');const b=event.target;b.innerHTML=`${icon('sparkles-outline')} Đang tạo…`;setTimeout(()=>{readEditorForm();t.value=genDesc(editing);b.innerHTML=`${icon('sparkles-outline')} AI mô tả`;toast('AI đã tạo mô tả');},700);},
  aiStory(){const t=$('#f-story');const b=event.target;b.innerHTML=`${icon('sparkles-outline')} Đang viết…`;setTimeout(()=>{readEditorForm();t.value=genStory(editing);b.innerHTML=`${icon('sparkles-outline')} AI viết chuyện`;toast('AI đã viết câu chuyện');},800);},
  pick(i){openPicker(i);},
  ...
  approveSpot(id){
    ...
    openModal(`<div class="mh"><h3>Duyệt đóng góp &amp; trả thưởng</h3><div class="x" onclick="closeModal()">${icon('close',17)}</div></div>
    <div class="mb">
      <div class="banner warn" style="margin-bottom:12px"><div class="bi">${icon('gift-outline',15)}</div><div>Khách sẽ nhận voucher cá nhân ngay lập tức kèm thông báo trong ứng dụng. Một gợi ý chỉ được thưởng một lần.</div></div>
      <div class="row2">
    ...
    <div class="mf"><button class="btn" onclick="closeModal()">Huỷ</button><button class="btn p" onclick="A.confirmApproveSpot('${escJs(id)}')">Duyệt &amp; gửi thưởng</button></div>`);
  },
  ...
  rejectSpot(id){
    openModal(`<div class="mh"><h3>Từ chối đóng góp?</h3><div class="x" onclick="closeModal()">${icon('close',17)}</div></div>
    <div class="mb"><p class="muted" style="font-size:13.5px">Khách sẽ nhận thông báo kèm lý do bên dưới để biết cần bổ sung gì. Không có voucher nào được phát.</p>
    ...
    <div class="mf"><button class="btn" onclick="closeModal()">Huỷ</button><button class="btn d" onclick="A.confirmRejectSpot('${escJs(id)}')">Từ chối</button></div>`);
  },
  ...
  /* Kiểm duyệt AI */
  async testMod(){const t=(document.getElementById('mod-test')||{}).value||'';const box=document.getElementById('mod-result');if(!t.trim()){box.innerHTML='<div class="faint" style="font-size:12.5px">Nhập nội dung để kiểm tra.</div>';return;}box.innerHTML='<div class="faint" style="font-size:12.5px">Đang kiểm tra…</div>';try{const d=await requestJSON('/moderation/test',12000,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:t})});const tone=d.decision==='rejected'?'b-red':d.decision==='pending'?'b-amber':'b-green';const label=d.decision==='rejected'?'BỊ CHẶN':d.decision==='pending'?'CHỜ DUYỆT':'HỢP LỆ';box.innerHTML=`<div class="banner ${d.decision==='rejected'?'err':d.decision==='approved'?'ok':'warn'}"><div class="bi">${d.decision==='rejected'?icon('ban-outline'):d.decision==='approved'?icon('checkmark-circle-outline'):icon('time-outline')}</div><div><b>${label}</b> · điểm ${Math.round((d.score||0)*100)}% · engine: ${esc(d.engine||'')}<div style="font-size:11.5px;margin-top:4px">${esc(d.reason||'')}</div>${(d.categories||[]).length?`<div style="margin-top:6px">${d.categories.map(c=>`<span class="bdg ${tone}" style="margin:0 4px 4px 0;display:inline-block">${esc(c)}</span>`).join('')}</div>`:''}${d.normalized?`<div class="faint" style="font-size:10.5px;margin-top:4px">Chuẩn hoá: <code>${esc(d.normalized)}</code></div>`:''}</div></div>`;}catch(e){box.innerHTML='<div class="faint" style="font-size:12.5px;color:var(--red,#c00)">Không kiểm tra được (backend/model chưa sẵn sàng).</div>';}},
  ...
  /* vouchers */
  addVoucher(){openModal(`<div class="mh"><h3>Tạo voucher</h3><div class="x" onclick="closeModal()">${icon('close',17)}</div></div><div class="mb">
  ...
  /* banners */
  addBanner(){openModal(`<div class="mh"><h3>Thêm ảnh quảng bá</h3><div class="x" onclick="closeModal()">${icon('close',17)}</div></div><div class="mb"><div class="field"><label>Tiêu đề</label><input id="bn-title" class="inp" placeholder="Bộ sưu tập mới"></div><div class="field"><label>Liên kết</label><input id="bn-link" class="inp mono" placeholder="/category/haori"></div><div class="field"><label>Màu nền</label><div class="chips">${['#8A2F26','#243244','#6B7255','#B08D3C','#6D28D9'].map(c=>`<div class="sw" onclick="A._bnc='${escJs(c)}';document.querySelectorAll('#bnc .sw').forEach(x=>x.style.outline='');this.style.outline='2px solid #A33A2F'" style="width:30px;height:30px;border-radius:6px;background:${c};cursor:pointer"></div>`).join('')}</div><div id="bnc"></div></div></div><div class="mf"><button class="btn" onclick="closeModal()">Huỷ</button><button class="btn p" onclick="A.saveBanner()">Thêm</button></div>`);A._bnc='#8A2F26';},
