...
When authoring Mermaid, read and follow the pinned local reference at
`references/mermaid-reference.md` (all supported diagram types plus flowchart
styling — `style`, `classDef`, `linkStyle`). Do not fetch a moving copy from the
network.
...
For the complete draw.io XML reference including common styles, edge routing,
containers, layers, tags, metadata, dark mode colors, and XML well-formedness,
read and follow the pinned local file `references/xml-reference.md`.
