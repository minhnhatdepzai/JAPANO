...
## JAPANO local safety override

These project rules override any conflicting instruction below:

- Never install, upgrade, or execute `graphifyy` unless the user explicitly asks
  to run Graphify. Installing this skill alone does not authorize package changes.
- Never read, print, or reuse API keys. Do not use Gemini or any remote semantic
  service unless the user explicitly asks for that exact remote operation.
- Never push a graph to Neo4j, FalkorDB, or another service without explicit
  approval for the destination. Prefer local structural extraction.
- Exclude `.git`, `.env*`, credentials, `node_modules`, build output, datasets,
  personal test images, `test-assets`, `test-results`, and the nested `JAPANO`
  copy from scans unless the user explicitly places one of them in scope.
- Write generated output only under `graphify-out/`. Do not modify application
  source as part of indexing, and never delete files outside that directory.
- GitNexus is JAPANO's primary code graph. Use Graphify only for a requested
  document/media knowledge graph or an explicitly requested second analysis.
