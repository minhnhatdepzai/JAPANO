            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {colors.map((c) => {
                ...
                if (isHex) {
                  ...
                  return (
                    <Pressable key={c} onPress={() => setSelectedColor(c)} style={{ width: 42, height: 42, borderWidth: active ? 3 : 1, borderColor: active ? theme.primary : theme.border, backgroundColor: raw, alignItems: 'center', justifyContent: 'center' }}>
                      {active ? <Feather name="check" size={18} color={light ? '#111' : '#fff'} /> : null}
                    </Pressable>
                  );
                }
                return (
                  <Pressable key={c} onPress={() => setSelectedColor(c)} style={pill(theme, active)}>
                    <Text style={pillText(theme, active)}>{c}</Text>
                  </Pressable>
                );
              })}
            </View>
