...
export const Btn = ({ label, onPress, variant='primary', style, icon }:
  { label:string; onPress?:()=>void; variant?:'primary'|'ghost'|'ink'; style?:ViewStyle; icon?:string }) => {
  const bg = variant==='primary'?C.shu: variant==='ink'?C.ai:'transparent';
  const fg = variant==='ghost'?C.shu:'#fff';
  return (
    <Pressable onPress={onPress} style={[s.btn,{ backgroundColor:bg, borderWidth:variant==='ghost'?1.5:0, borderColor:C.shu }, style]}>
