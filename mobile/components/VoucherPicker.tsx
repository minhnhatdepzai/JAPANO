...
export function VoucherField({ subtotal, voucher, onApply, onClear, userId }:{
  subtotal:number; voucher:AppliedVoucher|null; onApply:(v:AppliedVoucher)=>void; onClear:()=>void; userId?:string;
}) {
  ...
  const apply = async (rawCode:string) => {
    ...
    try {
      const res:any = await validateVoucher(value, subtotal, userId);
      if (res?.ok) {
