...
import { sendStylistMessage, trackInteraction } from './api';
...
export function BotChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const initialProducts = useMemo(() => topSuggestions(products), [products]);
  ...
  const send = useCallback(async (raw: string) => {
    ...
    try {
      ...
      addMessage({
        ...
        role: 'ai',
        text: result.message,
        productIds: result.products?.length ? result.products : initialProducts,
      });
    } catch {
      addMessage({
        id: messageId(),
  ...
  }, [addMessage, initialProducts, loading, messages]);
  ...
  const answerQuick = useCallback((label: string) => {
    ...
    void send(label);
  }, [addMessage, mode, navigate, products, removedItem, send]);
  ...
  const hiddenRoute = pathname === '/' || pathname === '/login' || pathname === '/register'
    || pathname === '/onboarding' || pathname === '/chat';
  const showFloating = isAuthenticated && ready && enabled && !hiddenRoute && !open;
            ...
            <View style={[styles.panel, { paddingBottom: Math.max(12, insets.bottom) }]}>
              <View style={styles.panelHeader}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.panelStatus}>● Đang hoạt động · thao tác trực tiếp</Text>
                </View>
