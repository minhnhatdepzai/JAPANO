...
import { trackInteraction, syncCart, AppliedVoucher } from './api';
...
export function StoreProvider({ children }:{ children:React.ReactNode }) {
  ...
  useEffect(()=>{
    ...
    AsyncStorage.getItem(`${STORAGE_KEY}/${encodeURIComponent(user.id)}`)
      .then(raw=>{
        if (!raw||!live) return;
        const saved = JSON.parse(raw);
        if (Array.isArray(saved?.wish)) setWish(saved.wish.map(String));
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(()=>{void syncCart(user.id,cart).catch(()=>undefined);},300);
