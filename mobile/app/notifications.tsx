...
export default function Notifications() {
  ...
  const onPullRefresh = () => { setRefreshing(true); void load({silent:true}); };
  ...
  const openNoti = async (n: Noti) => {
    ...
    if (!n.action) return;
    if (ROUTE[n.action]) { router.push(ROUTE[n.action] as any); return; }
    const orderMatch = n.action.match(/^order:(.+)$/);
    if (orderMatch) router.push(`/order/${orderMatch[1]}` as any);
  };
    ...
    <Screen>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 24 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onPullRefresh} tintColor={C.shu} colors={[C.shu]} />}>
        ...
        {filtered.map((n) => {
          ...
          const actionable = !!(n.action && (ROUTE[n.action] || /^order:/.test(n.action)));
          return (
