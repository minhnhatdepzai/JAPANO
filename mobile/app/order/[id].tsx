...
import { getOrderDetail, createProductReview, createReturnRequest, createCancelRequest, getProductReviews, ApiOrder, ReturnRequest, StripePaymentRecord } from '../../lib/api';
...
const RETURN_STATUS:Record<string,string>={requested:'Đang chờ duyệt',approved:'Đã duyệt · gửi hàng về cửa hàng',received:'Cửa hàng đã nhận hàng',refund_pending:'Đang hoàn tiền',refunded:'Đã hoàn tiền',refund_failed:'Hoàn tiền lỗi · đang xử lý',rejected:'Yêu cầu bị từ chối',cancelled:'Đã huỷ yêu cầu'};
...
const CANCEL_POLICY='Bạn có thể yêu cầu huỷ khi đơn CHƯA được bàn giao vận chuyển. Yêu cầu cần nêu lý do cụ thể và sẽ được cửa hàng xem xét — không huỷ ngay lập tức. Nếu được chấp nhận, đơn sẽ huỷ và số tiền đã thanh toán trực tuyến (nếu có) được hoàn theo đúng phương thức đã dùng. Nếu bị từ chối, đơn tiếp tục được xử lý và giao đến bạn như bình thường. Khi đơn đã bàn giao vận chuyển hoặc đã giao thành công, vui lòng dùng "Đổi/Trả hàng" thay vì huỷ.';
const RETURN_POLICY='Yêu cầu đổi/trả hàng áp dụng trong vòng 30 ngày kể từ khi đơn giao thành công. Bạn cần nêu lý do và đính kèm ít nhất 1 ảnh chụp thực tế sản phẩm/hàng hoá để cửa hàng xem xét. Quản trị viên sẽ duyệt hoặc từ chối: nếu duyệt, vui lòng gửi hàng về cửa hàng theo hướng dẫn — sau khi cửa hàng xác nhận đã nhận hàng, số tiền tương ứng được hoàn theo phương thức đã thanh toán (đơn thanh toán khi nhận hàng được hoàn thủ công). Nếu bị từ chối, sản phẩm không được hoàn tiền và được gửi trả lại bạn.';
...
export default function OrderDetail() {
  ...
  const canReturn=order.status==='completed'&&!returnRequest;
  const canCancel=CANCELLABLE_STATUSES.includes(order.status)&&!returnRequest;
  const submitReturn=async()=>{
    ...
    try{const result=await createReturnRequest(order.id,{reason,note,photos});setOrder(result.order);setReturnRequest(result.returnRequest);setReturnOpen(false);setPhotos([]);}
  ...
  const submitCancel=async()=>{
    ...
    try{const result=await createCancelRequest(order.id,{reason:cancelReason,note:cancelNote});setOrder(result.order);setReturnRequest(result.returnRequest);setCancelOpen(false);}
    catch(e:any){setCancelError(e?.message||'Không gửi được yêu cầu huỷ đơn.');}
  ...
  const submitReview=async()=>{
    ...
    try{const result=await createProductReview(slug,{userId:user.id,userName:user.name,rating:reviewRating,comment:reviewComment,media:reviewMedia?.dataUri,mediaKind:reviewMedia?.kind});setReviewEligibility(current=>({...current,[slug]:{canReview:false,alreadyReviewed:true}}));setReviewMessage(result.message);setReviewProduct(null);setReviewComment('');setReviewRating(5);setReviewMedia(null);}
    ...
    <Screen>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal:18, paddingBottom:24 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>void load()} tintColor={C.shu} colors={[C.shu]} />}>
        ...
        {!!returnRequest&&<View style={st.returnCard}>
          <View style={{flexDirection:'row',alignItems:'center',gap:9}}><Ionicons name={returnRequest.kind==='cancel'?'close-circle-outline':'return-down-back'} size={22} color="#6D28D9" /><View style={{flex:1}}><Text style={st.returnTitle}>{returnRequest.kind==='cancel'?'Yêu cầu huỷ đơn':'Trả hàng'} · {returnRequest.code}</Text><Text style={st.returnStatus}>{returnStatusLabel(returnRequest.status,returnRequest.kind)}</Text></View></View>
          <Text style={st.returnMeta}>Lý do: {returnRequest.reason}</Text>
          {returnRequest.kind==='return'&&<Text style={st.returnMeta}>Số tiền dự kiến: {money(returnRequest.amount)}</Text>}
          {!!returnRequest.refundId&&<Text selectable style={st.returnCode}>Mã hoàn tiền: {returnRequest.refundId}</Text>}
        </View>}
        {canCancel&&<View style={st.returnOffer}><View style={{flex:1}}><Text style={st.returnTitle}>Cần huỷ đơn?</Text><Text style={st.returnMeta}>Đơn chưa được bàn giao vận chuyển — bạn có thể gửi yêu cầu huỷ, cửa hàng sẽ xem xét và phản hồi sớm.</Text></View><Pressable style={[st.returnBtn,{backgroundColor:C.danger}]} onPress={()=>setCancelOpen(true)}><Text style={st.returnBtnT}>Huỷ đơn</Text></Pressable></View>}
        {canReturn&&<View style={st.returnOffer}><View style={{flex:1}}><Text style={st.returnTitle}>Cần trả hàng?</Text><Text style={st.returnMeta}>Gửi yêu cầu trong 30 ngày, kèm ảnh sản phẩm. Sau khi cửa hàng nhận hàng, khoản tiền sẽ được hoàn theo phương thức đã thanh toán.</Text></View><Pressable style={st.returnBtn} onPress={()=>setReturnOpen(true)}><Text style={st.returnBtnT}>Yêu cầu trả</Text></Pressable></View>}
      ...
      <Modal visible={returnOpen} transparent animationType="fade" onRequestClose={()=>setReturnOpen(false)}>
        <View style={st.modalShade}><View style={st.modalBox}>
          ...
          <ScrollView style={{maxHeight:460}} keyboardShouldPersistTaps="handled">
            <Text style={st.modalSub}>Chọn lý do. Yêu cầu sẽ được gửi tới trang quản trị để duyệt.</Text>
            ...
            <View style={st.policyBox}><Text style={st.policyTitle}>Chính sách đổi/trả hàng</Text><Text style={st.policyText}>{RETURN_POLICY}</Text></View>
