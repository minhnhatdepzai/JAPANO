...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const payStripeCheckout = async (extra: { total: number; shippingAddress?: string; paymentMethod?: string; checkoutMeta?: any }) => {
    ...
    // WEB: PaymentSheet của stripe-react-native không chạy trên web -> dùng Stripe Checkout redirect.
    if (Platform.OS === 'web') {
      const origin = (typeof window !== 'undefined' && window.location) ? window.location.origin + window.location.pathname : 'https://japano.app';
      const session = await api.createStripeCheckoutSession({
        ...
        metadata: { paymentMethod: extra.paymentMethod || 'Stripe', shippingAddress: extra.shippingAddress || user?.address || '' },
      });
      if (session?.url && typeof window !== 'undefined') {
        window.location.href = session.url;
        return { paymentIntentId: 'stripe_checkout_redirect' };
      }
      throw new Error(session?.message || 'Không tạo được phiên thanh toán Stripe cho web.');
    }
