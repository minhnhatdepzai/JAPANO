// JAPANO V54 FRONTEND REVIEW + SIZE COLOR SELECTORS
// @ts-nocheck
...
export default function ProductDetailV49ShopeeScreen() {
  ...
  useEffect(() => { if (product) setSelectedVariant(findSelectedVariant(product)); }, [product, selectedSize, selectedColor]);
  ...
  function requireSizeColor(actionName = 'tiếp tục') {
    const ss = productSizes(product);
    const cc = productColors(product);
    if (ss.length && !selectedSize) {
      Alert.alert('Chưa chọn size', `Vui lòng chọn size trước khi ${actionName}.`);
      return false;
    }
    if (cc.length > 1 && !selectedColor) {
      Alert.alert('Chưa chọn màu', `Vui lòng chọn màu sắc trước khi ${actionName}.`);
      return false;
    }
    return true;
  }
  ...
  function goTryOn() {
    if (!product) return;
    if (!requireSizeColor('thử đồ AI')) return;
    const variant = findSelectedVariant(product);
    router?.push?.({
      pathname: '/thu-do-ai-v49-shop-flow',
      params: {
        productId: v49ProductId(product),
        product: encodeURIComponent(JSON.stringify({ ...product, selectedSize, selectedColor, selectedVariantId: variant?.id || variant?._id || variant?.variantId || '' })),
        selectedSize,
        selectedColor,
  ...
  function addCart() {
    ...
    if (!requireSizeColor('thêm vào giỏ')) return false;
    const variant = findSelectedVariant(product);
    const nextItem = {
      ...product,
      id: v49ProductId(product),
      variantId: variant?.id || variant?._id || variant?.variantId || '',
      selectedVariantId: variant?.id || variant?._id || variant?.variantId || '',
      qty: 1,
    ...
    Alert.alert('Đã thêm vào giỏ', `${product?.name || 'Sản phẩm'} - Size ${selectedSize || recommendedSize || 'chưa chọn'} - Màu ${selectedColor || 'Mặc định'}`);
    return true;
  }

  function buyNow() {
    if (addCart()) router?.push?.('/checkout');
  }
      ...
      <ScrollView contentContainerStyle={s.container}>
        ...
        <View style={s.card}>
          ...
          <View style={s.selectionSummary}>
            ...
            <Text style={s.selectionSummarySub}>Lựa chọn này sẽ được dùng cho Thêm giỏ, Mua ngay và Thử đồ AI.</Text>
          </View>
        </View>
