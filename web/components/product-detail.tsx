...
export function ProductDetail({ product }: { product: Product }) {
  ...
  const tryOnHref = authStatus === "guest" ? loginHref(tryOnPath) : tryOnPath;
  ...
  return <div className="pdp-layout">
    ...
    <aside className="pdp-panel">
      <span className="eyebrow">{product.brand || "JAPANO"} · {product.kanji}</span><h1>{product.name}</h1><div className="pdp-rating"><span>{product.rating ? `★ ${product.rating} (${product.reviewCount || 0})` : "Chưa có đánh giá"}</span><span>Đã bán {Math.max(0, Number(product.sold || 0))}</span></div>
      ...
      <fieldset><legend>Kích cỡ <LinkSize href={authStatus === "guest" ? loginHref("/thu-do") : "/thu-do"} /></legend><div className="size-options">{sizes.map((variant) => <button key={variant.key} aria-pressed={size === variant.size} className={size === variant.size ? "active" : ""} onClick={() => setSize(variant.size)}>{variant.size}<small>{variant.stock <= 3 ? `Còn ${variant.stock}` : ""}</small></button>)}</div></fieldset>
      ...
      <a className="button tryon-button full" href={tryOnHref}><Sparkles aria-hidden="true" />Thử sản phẩm này trên ảnh của bạn</a>
...
function LinkSize({ href }: { href: string }) { return <a href={href}>Dùng AI gợi ý size</a>; }
