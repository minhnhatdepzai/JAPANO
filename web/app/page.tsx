...
export default async function HomePage() {
  ...
  return <>
    ...
    <section className="section"><SectionHeading eyebrow="Category · 装い" title="Chọn theo cách bạn muốn xuất hiện" />
    ...
    <section className="experience-split">
      ...
      <div className="tryon-art" aria-hidden="true"><span className="frame frame-one" /><span className="frame frame-two" /><span className="vertical-type">試 着 室</span></div>
    </section>

    <section className="section travel-home"><SectionHeading eyebrow="Wear JAPANO, visit Japan" title="Mặc JAPANO, đến Nhật Bản" copy="Thử sản phẩm trước, rồi đặt chính kết quả đó vào góc chụp đã duyệt — chân phải chạm đất, cảnh không nuốt mất con người." href="/du-lich-nhat-ban" linkLabel="Khám phá Nhật Bản" />
    ...
    <section className="section store-preview"><div><span className="eyebrow"><MapPin aria-hidden="true" /> JAPANO Store</span><h2>Chạm vải thật tại QTSC9.</h2><p>{home.shop.locations?.[0]?.address || home.shop.address}</p><p>{home.shop.locations?.[0]?.openingHours || "Liên hệ trước khi đến"} · {home.shop.hotline}</p><Link className="button secondary" href="/cua-hang">Xem bản đồ & chỉ đường</Link></div><div className="store-map-art" aria-hidden="true"><span className="route-line" /><span className="map-pin">J</span><small>10.8537915<br />106.6260636</small></div></section>

    <section className="service-row" aria-label="Chính sách mua hàng"><div><Truck aria-hidden="true" /><strong>Giao hàng minh bạch</strong><span>Phí được tính trước khi đặt</span></div><div><RefreshCw aria-hidden="true" /><strong>Đổi size trong 7 ngày</strong><span>Theo điều kiện công bố</span></div><div><ShieldCheck aria-hidden="true" /><strong>Thanh toán bảo vệ</strong><span>COD, Stripe và VNPay theo cấu hình thật</span></div></section>
  </>;
}
