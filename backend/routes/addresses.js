...
module.exports = function registerAddressRoutes(api, ctx) {
  const { read, update, httpError, requireAuth } = ctx;
  ...
  api.get('/addresses', requireAuth, (req, res) => {
  ...
  api.put('/addresses/:id', requireAuth, (req, res) => {
    try {
      const userId = req.user.id;
      const fields = readAddressBody(req.body);
      let updated;
  ...
  api.post('/addresses/:id/default', requireAuth, (req, res) => {
    try {
      const userId = req.user.id;
      update((state) => {
        const target = (state.addresses || []).find((a) => a.id === String(req.params.id) && a.userId === userId);
  ...
  api.delete('/addresses/:id', requireAuth, (req, res) => {
    try {
      const userId = req.user.id;
      update((state) => {
