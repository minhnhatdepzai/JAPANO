"""Run One-to-All Animation 1.3B-v2 for one JAPANO motion request.
...
def render_pose_sequence(reference_pose: dict, motion: str, width: int, height: int, frames: int):
...
def load_reference_pose(image: Image.Image, pose_root: Path) -> dict:
...
def main() -> int:
    ...
    checkpoint_root = repo / "checkpoints/One-to-All-1.3b_2"
    ...
    reference_pose = load_reference_pose(source, pose_root)
    control = render_pose_sequence(reference_pose, args.motion, width, height, args.frames)
